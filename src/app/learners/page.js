...
export default function Learners() {
  ...
  const [currentFilter, setCurrentFilter] = useState("All Learners");
  ...
  const handleDeleteConfirm = async () => {
    try {
      for (const leadId of selectedLeads) {
        await axios.delete(`${API_BASE_URL}/learner/${leadId}`);
      }
      fetchLeads();
      setSelectedLeads([]);
        ...
        <div className="relative">
          ...
          {isMyLeadsDropdownOpen && (
            <ul className="absolute top-full mt-1 bg-white shadow-lg rounded-lg z-40 w-full sm:w-auto">
              <li className="px-4 py-2 hover:bg-teal-200 cursor-pointer" onClick={() => handleFilterChange("All Learners")}>All Learners</li>
              <li className="px-4 py-2 hover:bg-teal-200 cursor-pointer" onClick={() => handleFilterChange("Today's Learners")}>Today&apos;s Learners</li>
              <li className="px-4 py-2 hover:bg-teal-200 cursor-pointer" onClick={() => handleFilterChange("Yesterday's Learners")}>Yesterday&apos;s Learners</li>
              <li className="px-4 py-2 hover:bg-teal-200 cursor-pointer" onClick={() => handleFilterChange("Previous Learners")}>Previous Learners</li>
            </ul>
          )}
        </div>
          ...
          <table className="min-w-full border border-teal-200 rounded-lg shadow-lg overflow-hidden">
            <thead className="bg-teal-100">
              <tr>
                ...
                <th className="p-2">Mode of Class</th>
              </tr>
            </thead>
              ...
              {loading ? (
                ...
                paginatedLeads.map((lead) => (
                  <tr key={lead.id} className="border-b border-b-teal-100 cursor-pointer" onClick={() => handleUpdateClick(lead.id)}>
                    ...
                    <td className="p-2 text-center">{formatDateTime(lead.createdAt).time}</td>
                    <td className="p-2 text-center">{formatDateTime(lead.createdAt).date}</td>
                    <td className="p-2 text-center">{lead.firstname} {lead.lastname}</td>
                    <td className="p-2 text-center">{lead.phone}</td>
                    <td className="p-2 text-center">{lead.email || '-'}</td>
                    <td className="p-2 text-center">{lead.modeOfClass || '-'}</td>
                  </tr>
                ))
              )}
            </tbody>
