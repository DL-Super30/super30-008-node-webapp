...
export default function Courses() {
  ...
  const [currentFilter, setCurrentFilter] = useState("All Courses");
  ...
  const handleUpdateClick = (leadId = null) => {
    ...
    // Case 3: Handle other cases (0 or multiple selections)
      ...
      toast.warn('Please select at least one course to update!', {
        position: "top-center",
        autoClose: 3000,
        theme: "colored",
      });
    } else {
      toast.error('Please select only one course to update at a time.', {
        position: "top-center",
        autoClose: 3000,
        theme: "colored",
      });
  ...
  const handleDeleteClick = () => {
    if (selectedLeads.length > 0) {
      ...
      toast.warn('Please select at least one course to delete!', {
        position: "top-center",
        autoClose: 3000,
  ...
  const handleDeleteConfirm = async () => {
    try {
      for (const leadId of selectedLeads) {
        await axios.delete(`${API_BASE_URL}/course/${leadId}`);
      }
      fetchLeads();
      setSelectedLeads([]);
    ...
    <div className="container mx-auto m-4 p-6 border shadow-lg rounded-2xl bg-slate-100">
        ...
        <div className="relative">
          ...
          {isMyLeadsDropdownOpen && (
            <ul className="absolute top-full mt-1 bg-white shadow-lg rounded-lg z-40 w-full sm:w-auto">
              <li className="px-4 py-2 hover:bg-teal-200 cursor-pointer" onClick={() => handleFilterChange("All Courses")}>All Courses</li>
              <li className="px-4 py-2 hover:bg-teal-200 cursor-pointer" onClick={() => handleFilterChange("Today's Courses")}>Today&apos;s Courses</li>
              <li className="px-4 py-2 hover:bg-teal-200 cursor-pointer" onClick={() => handleFilterChange("Yesterday's Courses")}>Yesterday&apos;s Courses</li>
              <li className="px-4 py-2 hover:bg-teal-200 cursor-pointer" onClick={() => handleFilterChange("Previous Courses")}>Previous Courses</li>
            </ul>
          )}
        </div>
      ...
      {view === "table" ? (
              ...
              {loading ? (
                ...
                paginatedLeads.map((lead) => (
                  <tr key={lead.id} className="border-b border-b-teal-100 cursor-pointer" onClick={() => handleUpdateClick(lead.id)}>
                    ...
                    <td className="p-2 text-center">{lead.courseName}</td>
                    <td className="p-2 text-center">{lead.description || '-'}</td>
                    <td className="p-2 text-center">{lead.courseFee || '-'}</td>
                  </tr>
                ))
              )}
            </tbody>
        ...
        <div className="flex flex-nowrap overflow-x-auto">
          {[
            { status: "Upcoming", headingColor: "text-yellow-500", bgColor: "bg-yellow-100", borderColor: "border-yellow-300" },
            { status: "Ongoing", headingColor: "text-green-500", bgColor: "bg-green-100", borderColor: "border-green-300" },
            { status: "Onhold", headingColor: "text-orange-500", bgColor: "bg-orange-100", borderColor: "border-orange-300" },
            { status: "Completed", headingColor: "text-red-500", bgColor: "bg-red-100", borderColor: "border-red-300" },
            ...
            <div key={status} className="flex-1 min-w-[250px] p-4 overflow-y-auto max-h-[500px]">
              <div className={`flex flex-col h-full border ${borderColor} rounded-lg shadow-lg bg-white`}>
                ...
                <p className="text-base font-semibold mb-2 pl-2">Total Fee Quoted: ₹ {filteredLeads
                  .filter((lead) => lead.learnerStage?.toLowerCase() === status.toLowerCase())
                  .reduce((total, lead) => total + (Number(lead.currency) || 0), 0)}
                </p>
                    ...
                    .map((lead) => (
                      <div key={lead.id} className={`border ${borderColor} rounded-lg p-3 mb-2 ${bgColor} overflow-x-auto`}>
                        <div className="flex flex-col sm:flex-row justify-between items-center">
                          <h3 className="font-semibold sm:pr-4">{lead.firstname}</h3>
                          <p className="text-sm text-gray-700">{lead.phone}</p>
                        </div>

                        <p className="text-sm text-gray-600 mt-1">{lead.techstack}</p>
                      </div>
                    ))}
                </div>
              </div>
